...
int on;
{
    if (on) {
        ...
        if (use_keys) {
            ...
            for (int i = 0; i < 9; ++i) {
                XGrabKey(dpy, XKeysymToKeycode(dpy, workspacekeys[i]), Mod4Mask,
                         root, False, GrabModeAsync, GrabModeAsync);
            }
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_r), Mod4Mask, root, False, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_t), Mod4Mask, root, False, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_z), Mod4Mask, root, False, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_x), Mod4Mask, root, False, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_d), Mod4Mask, root, False, GrabModeAsync, GrabModeAsync);
            XGrabKey(dpy, XKeysymToKeycode(dpy, XK_q), Mod4Mask|ShiftMask, root, False, GrabModeAsync, GrabModeAsync);
        }
...
Client *c;
{
    ...
    if (c == 0) {
        fprintf(stderr, "w9wm: active(c==0)\n");
        return;
    }
    if (c == current)
        return;
